...
2. **Built-in starter ruleset** (~150 common merchants + your specifics).
...
**History floor.** Reports never start earlier than your earliest **debit-account**
transaction — that's where income lands, so periods before it would show spending
with no matching income. The dashboard notes when a range is clamped.
...
| POST | `/transactions/{id}/category` | Re-categorize one transaction |
